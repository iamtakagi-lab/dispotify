    ...
    */
    ...
    return (
      <>
        {me ? (
          <>
            <p className="text-md inline pl-2">
              <a
                className="border border-green-500 rounded-full shadow-sm p-3 hover:bg-grey"
                href={`https://open.spotify.com/user/${me.spotifyUser.id}`}
              >
                <img
                  className="rounded-full inline mr-2"
                  width="32px"
                  src={me.spotifyUser.images[0].url}
                  alt=""
                />
                {me.spotifyUser.display_name || me.spotifyUser.id}
              </a>{' '}
              としてログインしています
            </p>
          </>
        ...
        <div className="mt-10">
          <p className="text-md">
            Discord Webhook URL を入力してください (複数の場合は , で区切る)
          </p>
          <p className="text-sm text-gray-500">
            e.g.
            https://discord.com/api/webhooks/xxx/xxx,https://discord.com/api/webhooks/xxx/xxx
          </p>
        ...
        <button
          className="h-8 px-4 m-2 text-sm text-blue-100 transition-colors duration-150 bg-blue-700 rounded-lg focus:shadow-outline hover:bg-blue-800"
          onClick={async () => {
            client
              .updateUser(webhookUrls, messageFormat)
              .then((data) => {
